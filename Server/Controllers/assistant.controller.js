...
export const getAssistantConfig = async (req, res) => {
    try {
        ...
        return res
            .status(500)
            .json({ message: `getAssistantConfig error ${error}` });
    }
};
...
export const askAssistant = async (req, res) => {
    try {
        ...
        if (!user.geminiApiKey) {
            return res
                .status(400)
                .json({ message: "gemini API key is not added" });
        }

        const decryptedApiKey = decrypt(user.geminiApiKey);
        ...
        if (user.enableNavigation && user.plan === "pro") {
            // Navigation commands
        ...
        return res
            .status(500)
            .json({ success: false, message: `askAssistant error ${error}` });
    }
};
